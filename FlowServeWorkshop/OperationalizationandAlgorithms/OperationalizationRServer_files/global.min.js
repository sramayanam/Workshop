...
var loc = {
    ...
    "disable": "Disable",
    "disclaimer.archive": "Draft: This documentation applies to an older version of the product.",
...
function fixDate() {
    $$1('time[datetime]').each(function () {
        try {
            ...
            $$1(this).attr('datetime', dateVal.toISOString());
...
function showDisclaimer(message, dismissable, link) {
    if (dismissable === void 0) { dismissable = false; }
    ...
    var alert = main.firstElementChild;
    if (link)
        alert.insertAdjacentHTML('beforeend', "<a class=\"button-border\" href=\"" + link.url + "\">" + link.text + "</a>");
    if (dismissable) {
        alert.insertAdjacentHTML('beforeend', "<button class=\"button-dismiss no-style\"><span class=\"visually-hidden\">" + loc["disclaimer.dismissAlert"] + "</span></button>");
    }
    return alert;
}
function setupDismissAlerts() {
    window.addEventListener('click', function (e) {
        var button = $(e.target).closest('.alert > .button-dismiss')[0];
        if (button === undefined) {
            return;
        }
...
function getPdfUrl(pdfName, pdfLink) {
    if (pdfLink === void 0) { pdfLink = null; }
    var urlTemplate = pdfLink === null ? getMeta('pdf_url_template') : pdfLink;
...
function renderPdfLink(pdfName, pdfLink) {
    if (pdfLink === void 0) { pdfLink = null; }
    var url = getPdfUrl(pdfName, pdfLink);
    if (url === null) {
        return;
    }
...
function createToc() {
    ...
    var getTocData = function (url, fallbackToMeta) {
        $$1.ajax({
            ...
            var pdfHref = getDataFromToc(tocJson, 'pdf_href');
            var pdfName = getDataFromToc(tocJson, 'pdf_name');
            var pdfLink = getDataFromToc(tocJson, 'pdf_url_template');
            if (pdfHref && pdfHref.length && pdfLink && pdfLink.length) {
                $$1(function () { return renderPdfLink(pdfHref, pdfLink); });
            }
            else if (pdfName && pdfName.length) {
                $$1(function () { return renderPdfLink(pdfName, null); });
            }
        })
            .fail(function () {
...
var cliPageUrl = cliPageOrigin + "/index.html?trustedAuthority=" + location$1.origin + "&embed=true&feature.azureconsole.ostype=";
...
function showArchiveDisclaimer() {
    if (isArchived) {
        ...
        if (archiveUrl) {
            ...
            showDisclaimer(loc['disclaimer.archive'], true, { url: archiveUrl, text: text });
        }
        else {
            var text = loc['disclaimer.returnToMain'];
            showDisclaimer(loc['disclaimer.archive'], true, { url: "https://docs.microsoft.com", text: text });
        }
    }
}
...
Promise.all([cookieConsent, contentLoaded])
    .then(function () {
    var contributorList = document$1.querySelectorAll('.contributors-text[hidden],.contributors[hidden],.contributors-label[hidden], .moreContributors[hidden]');
    for (var i = 0; i < contributorList.length; i++) {
        contributorList.item(i).hidden = false;
    }
    var imgs = document$1.querySelectorAll('.contributors img[data-src]');
